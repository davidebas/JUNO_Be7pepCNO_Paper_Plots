...
void Fig3_Subtracted() {	

	int maximumplot = 3e7;
	double minimumplot = 1;

	TFile f1("Inputs/OutputRoot_Test_BX_6Y.root","read");
	TH1D* h1 = (TH1D*)f1.Get("PseudoDataset_Sub")->Clone();
	
	TFile f2("Inputs/OutputRoot_Test_Ideal_6Y.root","read");
	TH1D* h2 = (TH1D*)f2.Get("PseudoDataset_Sub")->Clone();

	TFile f3("Inputs/OutputRoot_Test_Baseline_6Y.root","read");
	TH1D* h3 = (TH1D*)f3.Get("PseudoDataset_Sub")->Clone();

	TFile f4("Inputs/OutputRoot_Test_Worst_6Y.root","read");
	TH1D* h4 = (TH1D*)f4.Get("PseudoDataset_Sub")->Clone();

	TH1D* h_Be7 = (TH1D*)f1.Get("PDFs/Be7;1")->Clone();
	TH1D* h_pep = (TH1D*)f1.Get("PDFs/pep;1")->Clone();
	TH1D* h_CNO = (TH1D*)f1.Get("PDFs/CNO;1")->Clone();
  ...
  c1->Print("Fig3_Subtracted.pdf","pdf");
}
