...
void Fig4_Tagged() {	
	...
	TH1D* h1 = (TH1D*)f.Get("PseudoDataset_Tag")->Clone();
	f.cd("PDFs");
	TH1D* h2 = (TH1D*)f.Get("PDFs/Be7;2")->Clone();
	TH1D* h3 = (TH1D*)f.Get("PDFs/pep;2")->Clone();
	TH1D* h4 = (TH1D*)f.Get("PDFs/CNO;2")->Clone();
	TH1D* h5 = (TH1D*)f.Get("PDFs/Bi210;2")->Clone();
	TH1D* h6 = (TH1D*)f.Get("PDFs/K40;2")->Clone();
	TH1D* h7 = (TH1D*)f.Get("PDFs/Kr85;2")->Clone();
	TH1D* h8 = (TH1D*)f.Get("PDFs/U238;2")->Clone();
	TH1D* h9 = (TH1D*)f.Get("PDFs/Th232;2")->Clone();
	TH1D* h10 = (TH1D*)f.Get("PDFs/Po210;2")->Clone();
	TH1D* h11 = (TH1D*)f.Get("PDFs/C11_2;2")->Clone();

	TH1D* h12 = (TH1D*)f.Get("PDFs/C10_2;2")->Clone();
	TH1D* h13 = (TH1D*)f.Get("PDFs/He6_2;2")->Clone();
	...
	h12->SetLineColor(922); h12->SetLineWidth(2); h12->SetLineStyle(3);	
	h12->DrawCopy("same");

	h13->SetLineColor(862); h13->SetLineWidth(2); h13->SetLineStyle(3);	
	h13->DrawCopy("same");
	...
	leg3->AddEntry(h12,"^{10}C","l");
	leg3->AddEntry(h13,"^{6}He","l");	
